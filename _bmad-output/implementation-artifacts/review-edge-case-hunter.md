...
Use `$bmad-review-edge-case-hunter` in a separate session and follow these rules exactly:

- Output language must be Chinese.
- The diff below is the primary input. You may inspect repository files read-only if needed.
- Systematically probe boundary conditions, failure paths, cross-platform behavior, zero values, initialization order, CLI/config interactions, release build behavior, and missing tests.
- Do not include praise or summary text.
- If there are no actionable issues, output exactly: `未发现可操作问题`.
- Output format: a Markdown list. Each finding must include: title, severity, trigger scenario, evidence, impact, and one-line fix direction.
...
- Reviewer layer: `edge-case-hunter`
- Context access: `diff + read-only project access`
