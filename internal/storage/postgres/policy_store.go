...
func (s *PolicyStore) SaveBundle(bundle *policy.PolicyBundle) error {
	...
	`, bundle.BundleID, bundle.Name, bundle.Version, cg, pt, ar, rt, dhr, bundle.CreatedAt)
	if err != nil {
		return err
	}

	var count int
	_ = s.pool.QueryRow(ctx(), `SELECT COUNT(*) FROM policy_bundles WHERE is_active`).Scan(&count)
	if count == 0 {
		_, _ = s.pool.Exec(ctx(), `UPDATE policy_bundles SET is_active = TRUE WHERE bundle_id = $1`, bundle.BundleID)
	}
	return nil
}
...
func (s *PolicyStore) GetBundle(bundleID string) (*policy.PolicyBundle, error) {
	...
	err := s.pool.QueryRow(ctx(), `
		...
		FROM policy_bundles WHERE bundle_id = $1
	`, bundleID).Scan(&b.BundleID, &b.Name, &b.Version, &cg, &pt, &ar, &rt, &dhr, &b.CreatedAt)
	if err != nil {
...
func (s *PolicyStore) ListBundles() ([]*policy.PolicyBundle, error) {
	rows, err := s.pool.Query(ctx(), `SELECT bundle_id FROM policy_bundles ORDER BY created_at`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (s *PolicyStore) GetActiveBundle() (*policy.PolicyBundle, error) {
	var bundleID string
	err := s.pool.QueryRow(ctx(), `SELECT bundle_id FROM policy_bundles WHERE is_active LIMIT 1`).Scan(&bundleID)
	if err != nil {
		if err == pgx.ErrNoRows {
			return nil, nil
		}
...
func (s *PolicyStore) SetActiveBundle(bundleID string) error {
	_, err := s.pool.Exec(ctx(), `UPDATE policy_bundles SET is_active = FALSE`)
	if err != nil {
		return err
	}
	cmd, err := s.pool.Exec(ctx(), `UPDATE policy_bundles SET is_active = TRUE WHERE bundle_id = $1`, bundleID)
	if err != nil {
		return err
	}
